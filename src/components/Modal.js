       ...
       <div onClick={(e) => { e.stopPropagation()}}className='modalcontainer'>
        ...
        <div className='modalRight'>
            ...
            <div className='content'>
                <h1 className='modalHeading'>Add Elderly</h1>
                <input onChange={(event) => {setNewid(event.target.value);}} placeholder='ID Number'/>
                {idError&&<div className='msg-error-modal'>{idError}</div>}
                <input onChange={(event) => {setNewElderly(event.target.value);}} placeholder='Elderly Full Name'/>
